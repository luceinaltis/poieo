...
DOCS = ROOT / "docs"
SKIP = {".claude", ".git", "node_modules", "worktrees", "dist"}
...
def test_every_module_points_at_a_design_document_that_exists():
    """Modules use this as the thread
...
def test_design_documents_are_not_dated():
    """Current reasoning belongs in a component document; git keeps history."""
    dated = [p.relative_to(ROOT) for p in DOCS.rglob("*.md") if DATED.match(p.name)]
    assert not dated, f"dated design documents belong in git history: {dated}"
