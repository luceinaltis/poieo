...
def test_an_exhausted_connection_reads_as_unreachable():
    cause = explain_failure(
        _wrap(ProviderError("local: cannot reach http://localhost:11434: refused", provider="local", retryable=True))
    )
...
def test_a_missing_key_reads_as_no_credentials():
    cause = explain_failure(_wrap(ProviderError("provider 'claude': $ANTHROPIC_API_KEY is not set")))
    assert cause.slug == "no_credentials"


def test_a_4xx_reads_as_rejected():
    cause = explain_failure(_wrap(ProviderError("local: HTTP 404: model 'nope' not found", retryable=False)))
...
def test_a_retryable_429_is_unreachable_not_rejected():
    ...
    cause = explain_failure(_wrap(ProviderError("claude: HTTP 429: overloaded", retryable=True)))
    assert cause.slug == "unreachable"


def test_max_turns_reads_as_out_of_turns():
    cause = explain_failure(NodeError("node 'work' hit max_turns (40) with tool calls still pending", node_id="work"))
...
def test_being_cut_off_reads_as_out_of_room_and_names_the_budget():
    ...
    cause = explain_failure(
        NodeError(
            "node 'work' was cut off before it finished: the model reached its output limit mid-turn", node_id="work"
        )
    )
    ...
    # And it points at `effort`, not at a token budget for the thinking. The
    # latter is what this used to say and it was wrong: an endpoint that only
    # takes an effort level converts the number into one by its share of
    # max_tokens, so `reasoning: {max_tokens: 16000}` under a 24,000 ceiling
    # asks for medium effort rather than a 16,000-token lid. Naming the thing
    # that is actually being chosen is the whole fix.
    assert "effort" in cause.fix


def test_unparseable_output_reads_as_bad_output():
    cause = explain_failure(NodeError("node 'a' expected JSON output but got: 'sure! here you go'"))
    assert cause.slug == "bad_output"


def test_a_missing_output_path_reads_as_bad_output():
    cause = explain_failure(NodeError("node 'a': output path 'x.y' is missing from the parsed JSON"))
    assert cause.slug == "bad_output"


def test_a_vanished_workdir_reads_as_folder_gone():
    cause = explain_failure(NodeError("node 'a': workdir does not exist: /tmp/gone"))
...
def test_max_steps_reads_as_cycling():
    cause = explain_failure(RunAborted("exceeded max_steps (50); the graph may be cycling without an exit condition"))
...
def test_run_prints_the_cause_and_the_fix(tmp_path):
    ...
    result = runner.invoke(app, ["run", str(graph), "-b", str(binding), "--no-log"])
...
def test_the_summary_row_carries_the_cause(tmp_path):
    graph, binding = _write(tmp_path)
    store = tmp_path / "logs"
    result = runner.invoke(app, ["run", str(graph), "-b", str(binding), "--store", str(store)])
    assert result.exit_code == 1
    row = json.loads((store / "index.jsonl").read_text(encoding="utf-8").splitlines()[-1])
