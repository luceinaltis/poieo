...
첫 화면의 왼쪽 여백에는 금빛 해와 옅은 초승달을 둔다. 방문자 기기의 현지 시각으로
06–18시는 해, 18–06시는 달을 표시하며 각 12시간의 중간에 가장 높이 뜬다.
실제 일출·일몰이나 달의 위상을 계산하는 기능은 아니다. 위치 권한이나 외부 요청 없이
분마다 갱신하고, 숨긴 탭은 갱신을 멈췄다가 돌아오면 현재 시각을 다시 읽는다.
모바일에서는 제목 위로 작게 옮기고, 동작 줄이기 설정에서는 이동 전환 효과를 끈다.
해와 달은 다크·라이트 테마 선택과 독립적이며, 추가 문구나 조작부를 만들지 않는다.
