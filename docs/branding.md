...
하늘에는 구름 세 덩이가 해와 달 앞을 천천히 흘러가며, 지나갈 때 해와 달을 가린다.
구름은 [brand/clouds.py](../brand/clouds.py)가 노이즈로 렌더링한 반투명 이미지다. 밝은 화면용은
햇빛을 받은 흰 구름, 어두운 화면용은 달빛의 회색 구름이며 모두 왼쪽 위에서 빛을 받으므로
CSS로 뒤집거나 색을 바꾸지 않는다.
