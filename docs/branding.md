...
첫 화면은 수묵담채의 하늘이다. 해는 옅은 주홍 안료가 번진 원, 달은 먹빛 속에 남긴 종이색 여백으로 그린 SVG를 쓴다.
밝은 화면에는 해, 어두운 화면에는 달이다. Auto에서는 방문자 기기의 현지 시각으로
06–18시가 밝은 화면이므로 해가, 나머지 시간에는 달이 뜬다. Light·Dark를 직접 고르면
시각과 상관없이 그 화면의 해나 달을 보여 준다. 어느 쪽이든 호는 시각을 따라
왼쪽에서 떠서 정오·자정에 가운데 가장 높은 곳을 지나 오른쪽으로 내려간다.
하늘에는 수묵 담채의 구름 띠 세 개가 해와 달 앞을 천천히 흘러가며, 지나갈 때 안개처럼
해와 달을 가린다. 구름은 [brand/clouds.py](../brand/clouds.py)가 감나무 담채와 같은 방식으로
렌더링한 반투명 이미지다. 한 가지 먹색에 농담은 투명도로 두고, 아랫면으로 먹이 모이며
윗부분은 종이 속으로 옅어진다. 밝은 화면에는 묽은 먹을, 어두운 화면에는 같은 번짐을 옅은
아이보리로 쓴다. 구름을 세로로 압축하고 농도를 낮춰 얇은 안개 띠로 천천히 흘린다.
밤에는 회청색 먹빛 하늘에 넓고 옅은 빛이 스며든다. 사진의 표면과 날카로운 후광은 쓰지 않는다. 가지도 채도와 농도를 낮춰 같은 그림에 어우러지게 한다.
실제 일출·일몰이나 달의 위상을 계산하는 기능은 아니다. 위치 권한이나 외부 요청 없이
분마다 갱신하고, 숨긴 탭은 갱신을 멈췄다가 돌아오면 현재 시각을 다시 읽는다.
모바일에서도 제목 위의 여백을 가로질러 이동한다. 제목과 메뉴를 가리지 않으며,
동작 줄이기 설정에서는 위치를 즉시 갱신하고 구름도 멈춘 채로 둔다. 해·달 교체나 오래 닫아 둔 탭의
복귀 때는 화면을 가로지르는 전환 효과를 생략한다. 현재 시각에 필요한 이미지 하나만 읽는다.
이전 사진형 해·달의 원본과 생성 프롬프트는 [생성 기록](../brand/sky-source.json)에 보존한다.
