...
사용자가 정한 전제와 제안을 구분한다. **2안 ‘담채와 열매’의 영문 시안을
선택했다. 이 방향에 맞춘 새 심벌과 워드마크를 탐색 중이며, 현재 랜딩·문서
페이지·README에는 아직 적용하지 않았다.**
제품의 실제 동작과 한계는 [DESIGN.md](../DESIGN.md)를 따른다.

## 정해진 전제

- 작은 모델과 큰 모델을 조합해 **적은 비용으로 높은 정확도**를 목표로 한다.
- **나무와 황금색 열매**의 이야기를 이어 가며 심벌과 워드마크를 새로 탐색한다.
- 한국적인 정서를 살릴 나무로 **감나무·매화·느티나무**를 비교한다. 수종은 아직 정하지 않았다.
...
제목은 시안에서 두 줄로 나눈다. 구현할 때 이 문구를 기준으로 삼는다.
...
나무와 황금색 열매의 의미, 먹색과 황금색을 이어 가되 심벌과 글자 모양은
선택한 담채 화면에 맞게 새로 탐색한다. 심벌에 부여할 의미는
...
선택한 담채 화면에 맞춰 심벌과 소문자 **poieo**를 함께 새로 그린 비교 시안이다.
원래 로고의 기하학적인 글자와 둥근 가지 끝을 바꾸고, 나무·열매와 라틴 세리프의
획을 연결한다. 이름은 다섯 글자가 분명하게 읽혀야 한다. 감나무는 금빛 열매를
이어 가고, 매화와 느티나무는 꽃과 잎으로 금빛 포인트를 바꾼 대안이다.

### 현재 비교안 · 감나무, 매화, 느티나무

B안의 세리프 워드마크와 먹색·밝은 바탕을 공통 기준으로 삼아 나무의 인상을 비교한다.
아래 설명은 디자인 의도이며 사용자가 선택한 최종 로고는 아니다.

| 안 | 형태와 금색 포인트 | 의도 |
|---|---|---|
| 감나무 | 넓은 잎 세 장, 휘어진 가지, 금빛 감 한 알 | 담백하고 친숙한 인상. 기존 열매의 의미와 연결 |
| 매화 | 성긴 가지, 작은 꽃 세 송이, 금빛 꽃 한 송이 | 섬세하고 가벼운 인상 |
| 느티나무 | 굵은 줄기, 넓게 벌어지는 가지, 금빛 잎 | 든든하고 안정적인 인상 |

**감나무**

![감나무 로고 시안: 넓은 잎과 금빛 감 한 알, 세리프 poieo 워드마크](assets/branding/logo-persimmon-concept.png)

**매화**

![매화 로고 시안: 성긴 가지와 작은 꽃, 세리프 poieo 워드마크](assets/branding/logo-plum-concept.png)

**느티나무**

![느티나무 로고 시안: 넓게 펼쳐진 가지와 금빛 잎, 세리프 poieo 워드마크](assets/branding/logo-zelkova-concept.png)

세 안은 ChatGPT Image의 기본 이미지 생성 도구로 만들었다.
사용한 참조 이미지와 전체 프롬프트는 [생성 기록](assets/branding/logo-tree-prompts.json)에 남긴다.
...
최종 로고 선택은 아직 확정하지 않았다. 시트의 분리형·단색 표시는 조합을 살펴보기 위한 생성 시안이다.
같은 도형을 정확히 축소한 납품 파일은 아니다. 선택한 안을 벡터로 정리한 뒤
심벌, 워드마크, 가로 조합, 단색 조합을 동일한 원본에서 만들고 작은 크기의 가독성을 확인한다.
