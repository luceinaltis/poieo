...
배경에는 달·별·구름을 그려 넣지 않고 별도 레이어로 둔다. 밝은 화면은 종이와 담채 가지를 유지한다.
...
차오를 때 오른쪽, 기울 때 왼쪽을 밝히는 곡선 마스크로 원본 달 표면을 드러낸다.
명암 경계를 부드럽게 하고 어두운 면을 희미하게 남겨 둥근 달의 입체감을 유지한다.
마스크의 번짐은 원 안으로 제한해 원본 후광이 밝은 테두리로 나타나지 않게 한다.
합삭에는 밝은 원과 보름달 후광을 표시하지 않는다. 화면의 설명에도 근사 위상·밝은 면적을 남긴다.
