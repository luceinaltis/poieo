...
첫 화면의 하늘에는 ChatGPT Image로 만든 사실적인 해와 보름달을 둔다.
...
아이보리로 쓴다. CSS로 색을 바꾸거나 선명하게 하지 않는다.
밤에는 달 주변으로 옅은 아이보리 빛이 퍼지고, 표면의 질감은 선명하게 유지한다.
...
이미지 원본과 생성 프롬프트는 [생성 기록](../brand/sky-source.json)에 남긴다.
