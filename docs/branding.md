...
ChatGPT Image로 만든 비교 시안이다. **1안 ‘먹과 여백’을 우선 제안한다.**
기존 로고의 먹색과 황금색을 이어 가며 여백과 붓의 흔적을 사용한다.
그림은 첫 화면의 한 부분에 두고, 문서 본문과 작업 화면은 읽기 편하게 유지한다.

### 1안 · 먹과 여백

왼쪽의 짧은 제목과 오른쪽 아래의 작은 수묵 나무를 조합한다. 성긴 먹선,
넓은 여백, 한 점의 황금색으로 절제된 인상을 만든다. 제목에는 현대적인
바탕 계열 글꼴을 시험하고, 본문과 버튼은 단정한 산세리프를 사용한다.

![먹과 여백: 짧은 제목과 성긴 수묵 나무를 사용한 랜딩 컨셉](assets/branding/joseon-ink-concept.png)

### 2안 · 담채와 열매

가운데 정렬한 제목과 화면 가장자리에서 들어오는 옅은 가지를 조합한다.
먹의 농도를 낮추고 열매와 버튼에 황금색을 사용한다. 더 부드러운 인상이지만
식물 그림이 제품보다 강하게 기억되지 않도록 실제 적용에서는 그림의 비중을 조절한다.

![담채와 열매: 옅은 가지와 황금색 열매를 사용한 랜딩 컨셉](assets/branding/joseon-wash-concept.png)
