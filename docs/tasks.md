...
through the task's `/approve` route, which the chat's timeline answers with
allow and deny. The chat's permission picker writes this key and `tools`
together, as one of four settings -- see `web.md`. Only a chat card takes the key -- a task that runs unwatched
has nobody to ask.
